...
impl Line {
    ...
    fn check_intersect_rect(&self, rect: &Rect) -> bool {
        match self {
            Self::Horizontal { x1, x2, y } => {
                if *y < rect.y0 || rect.y1 < *y {
                    false
                // } else if *y == rect.y0 || *y == rect.y1 {
                //     // check line should be longer than the rect horizontal sides
                //     !(*x1 <= rect.x0 && rect.x1 <= *x2)
                } else {
                    // either point cannot be inside
                    (rect.x0 < *x1 && *x1 < rect.x1) || (rect.x0 < *x2 && *x2 < rect.x1)
                }
            }
            Self::Vertical { x, y1, y2 } => {
                ...
                //     !(*y1 <= rect.y0 && rect.y1 <= *y2)
                } else {
                    // either point cannot be inside
                    (rect.y0 < *y1 && *y1 < rect.y1) || (rect.y0 < *y2 && *y2 < rect.y1)
                }
            }
        }
    }
...
fn main() -> Result<()> {
    ...
    for i in 0..cords.len() {
        for j in (i + 1)..cords.len() {
            let l = cords[i];
            let r = cords[j];
            let rect = Rect::new(l.clone(), r.clone());
            if !lines.iter().any(|l| l.check_intersect_rect(&rect))
                && [(l.0, r.1), (r.0, l.1)].iter().all(|p| {
                    lines
                        .iter()
                        .filter(|l| l.check_intersection(p.clone()))
                        .count()
                        % 2
                        == 1
                })
